...
void printTheObFile(struct machineCode* machineHead,char *fileName,int *IC,int *DC){
    ...
    size_t fileNameLength = strlen(copiedString);
    if (fileNameLength >= 3) {
        copiedString[fileNameLength - 3] = '\0';
    }
    snprintf(fileNameOb, sizeof(fileNameOb), "%s.ob", copiedString);
...
void printTheEntry(char* fileName,struct dataTable* dataHead,char* type){
    ...
    snprintf(fileNameEnt, sizeof(fileNameEnt), "%s.ent", copiedString);
...
void printExtern(char* fileName,struct machineCode* machineHead,struct dataTable* dataHead,char *type){
    ...
    snprintf(fileNameExt, sizeof(fileNameExt), "%s.ext", copiedString);
...
void makeTheEntryAndExtern(struct dataTable* dataHead,struct machineCode* machineHead,char *fileName,char *type,char* extenstion){
    ...
    snprintf(fileNameOb, sizeof(fileNameOb), "%s.%s", fileNameCopy,extenstion);
