...
int opcode(char line[], int ind ,struct  machineCode* dhead) /* Return the index of the opcode, and update the funct in the decode table */
{
    int i = 0;
...
int firstcheck(char* fileName, struct  dataTable* dataHead, struct  machineCode* machineHead, int *IC, int *DC){
    int tempDC = 0 , tempIC = 100,ind,flag=1,value ;
    ...
    while (fgets(line, MAX_LINE_LENGTH, file))
    {
        ...
        if(isSymbol(line,temp,tail,dataHead, machineTemp))
        {
            ...
            else
            {
                ...
                tempIC = tempIC + Instruction(line,index,machineTemp);
            }
